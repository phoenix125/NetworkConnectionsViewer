#AutoIt3Wrapper_outfile=ConnView.exe
...
Global $aUtilName = "Network Connections Viewer"
Global $aStartText = "Network Connections Viewer by trancexx" & @CRLF & "Modified by Phoenix125.com" & @CRLF& @CRLF
...
Global $aStartSplash = _Splash($aStartText, 0, 475, 140)
ControlSetText($aStartSplash, "", "Static1", $aStartText & "Getting privileges.")
_CV_GetPrivilege_SEDEBUG()
Global $iIsAdmin = IsAdmin()

ControlSetText($aStartSplash, "", "Static1", $aStartText & "Getting system module information.")
Global Const $sSystemModule = _CV_SystemModuleInformation()

Global $hTimer ; for ToolTip
HotKeySet("{F5}", "_CV_ShakeIt")
...
ControlSetText($aStartSplash, "", "Static1", $aStartText & "Getting connections.")
_CV_GetConnections($aTCPArray, $aUDPArray, $aStartSplash)

Global $hListViewItemTCP[UBound($aTCPArray)]
Global $hListViewItemUDP[UBound($aUDPArray)]

Global $hGUI = GUICreate("Network Connections Viewer", 820, 500, -1, -1, 0xCF0000) ; WS_OVERLAPPEDWINDOW
Global $hTab = GUICtrlCreateTab(10, 10, 800, 420)
GUICtrlSetResizing(-1, 102)

ControlSetText($aStartSplash, "", "Static1", $aStartText & "Creating drag dots.")
Global $hDots = _CV_CreateDragDots($hGUI)

Global $hTabTCP, $hListViewTCP
Global $hTabUDP, $hListViewUDP

Global $iUBoundListViewItemTCP, $iUBoundListViewItemUDP

ControlSetText($aStartSplash, "", "Static1", $aStartText & "Creating tabs.")
_CV_MakeTabs()
...
Global $hButtonCopy = GUICtrlCreateButton("&Copy", 180, 450, 100, 27)
GUICtrlSetTip(-1, "Copy data about the connection to the clipboard")
GUICtrlSetState(-1, 128) ; GUI_DISABLE

Global $hMyLabel = GUICtrlCreateLabel(BinaryToString("0x7472616E63657878206D616465207468697321"), 303, 450, 120, 30)
GUICtrlSetFont(-1, 10, 600, 1, "Courier New", 5)
...
GUICtrlSendMsg(-1, 5644, 0, 1) ; BCM_SETSHIELD
If $iIsAdmin Then GUICtrlSetState(-1, 128) ; GUI_DISABLE

Global $hButtonClose = GUICtrlCreateButton("Cl&ose", 670, 450, 100, 27)
GUICtrlSetTip(-1, "Close this window")

GUIRegisterMsg(5, "_CV_AdjustPos") ; WM_SIZE
GUIRegisterMsg(36, "_CV_SetMinMax") ; WM_GETMINMAXINFO

GUISetState()
WinActivate($hGUI)

Global $iMsg, $aCurInfo, $iRead
Global $iShootItOut, $iCopyEnabled
SplashOff()
; The main loop
While 1
	$iMsg = GUIGetMsg()
	Switch $iMsg
		...
		Case $hButtonDisable
			...
			_CV_DisableConnection(GUICtrlRead($iShootItOut))
			ContinueCase
		Case $hButtonRefresh
			_CV_GetConnections($aTCPArray, $aUDPArray)
			_CV_MakeTabs()
			ContinueCase
		Case -7
			ToolTip("")
			...
			If GUICtrlRead($hTab) = 0 Then
				$iRead = GUICtrlRead($hListViewTCP)
				For $i = 0 To $iUBoundListViewItemTCP
					If $iRead = $hListViewItemTCP[$i] Then
						If Not $iCopyEnabled Then GUICtrlSetState($hButtonCopy, 64) ; GUI_ENABLE
						$iCopyEnabled = $iRead
						If StringInStr(GUICtrlRead($iRead), "ESTABLISHED", 1) Then
							$iShootItOut = $iRead;$hListViewItemTCP[$iRead]
							GUICtrlSetState($hButtonDisable, 64) ; GUI_ENABLE
						...
						ExitLoop
					Else
						If $iCopyEnabled And $aCurInfo[4] <> $hButtonCopy Then
							GUICtrlSetState($hButtonCopy, 128) ; GUI_DISABLE
							$iCopyEnabled = 0
						EndIf
						If $iShootItOut And $aCurInfo[4] <> $hButtonDisable Then
							GUICtrlSetState($hButtonDisable, 128) ; GUI_DISABLE
							$iShootItOut = 0
						EndIf
					EndIf
				Next
			Else
				$iRead = GUICtrlRead($hListViewUDP)
				For $i = 0 To $iUBoundListViewItemUDP
					If $iRead = $hListViewItemUDP[$i] Then
						If Not $iCopyEnabled Then GUICtrlSetState($hButtonCopy, 64) ; GUI_ENABLE
						$iCopyEnabled = $iRead
						ExitLoop
					Else
		...
		Case $hTab
			...
			If GUICtrlRead($hTab) = 0 Then
				GUICtrlSetState($hButtonDisable, 16) ; $GUI_SHOW
			Else
				GUICtrlSetState($hButtonDisable, 32) ; $GUI_HIDE
			EndIf
		Case $hButtonRunAs
			...
			EndIf
		Case $hButtonCopy
			If $iCopyEnabled Then
				ClipPut(_CV_Format(GUICtrlRead($iCopyEnabled)))
				ConsoleWrite(_CV_Format(GUICtrlRead($iCopyEnabled)) & @CRLF)
				GUICtrlSetTip($hButtonCopy, "")
				ToolTip("Information about connection is written to the cliboard", Default, Default, "Copied", 1)
				DllCall($hUSER32, "bool", "MessageBeep", "dword", 64) ; MB_ICONINFORMATION
				$hTimer = TimerInit()
			EndIf
	EndSwitch

	If $hTimer Then
		If TimerDiff($hTimer) > 1500 Or WinGetHandle("[ACTIVE]") <> $hGUI Then
			GUICtrlSetTip($hButtonCopy, "Copy data about the connection to the clipboard")
			ToolTip("")
...
Func _CV_MakeTabs()
	...
	$hListViewTCP = GUICtrlCreateListView(_CV_GetColumns($aTCPArray, 0), 15, 37, $aClientSize[0] - 33, $aClientSize[1] - 113)
	...
	For $i = 1 To UBound($aTCPArray) - 1
		$hListViewItemTCP[$i - 1] = GUICtrlCreateListViewItem(_CV_GetColumns($aTCPArray, $i), $hListViewTCP)
		If $iExtendedTCP Then
			;If $aTCPArray[$i][7] <> "-" Then
			If Not GUICtrlSetImage(-1, $aTCPArray[$i][7], 0, 1) Then
				If Not GUICtrlSetImage(-1, "user32.dll", 100, 0) Then GUICtrlSetImage(-1, "shell32.dll", 3, 0)
			EndIf
			;EndIf
		EndIf
	Next
	...
	$hListViewUDP = GUICtrlCreateListView(_CV_GetColumns($aUDPArray, 0), 15, 37, $aClientSize[0] - 33, $aClientSize[1] - 113)
	...
	For $i = 1 To UBound($aUDPArray) - 1
		$hListViewItemUDP[$i - 1] = GUICtrlCreateListViewItem(_CV_GetColumns($aUDPArray, $i), $hListViewUDP)
		If $iExtendedUDP Then
			;If $aUDPArray[$i][4] <> "-" Then
			If Not GUICtrlSetImage(-1, $aUDPArray[$i][4], 0, 1) Then
				If Not GUICtrlSetImage(-1, "user32.dll", 100, 0) Then GUICtrlSetImage(-1, "shell32.dll", 3, 0)
			EndIf
			;EndIf
		EndIf
	...
	$iUBoundListViewItemUDP = UBound($hListViewItemUDP) - 1 ; global var

	Return 1

EndFunc   ;==>_CV_MakeTabs


Func _CV_GetConnections(ByRef $aTCPArray, ByRef $aUDPArray, $tSplash = 0)
	$aTCPArray = _CV_GetExtendedTcpTable($tSplash)
	$iExtendedTCP = 1
	If @error Then
		$iExtendedTCP = 0
		$aTCPArray = _CV_GetTcpTable($tSplash)
		If @error Then Exit -2
	EndIf
	$aUDPArray = _CV_GetExtendedUdpTable($tSplash)
	$iExtendedUDP = 1
	If @error Then
		$iExtendedUDP = 0
		$aUDPArray = _CV_GetUdpTable($tSplash)
...
Func _CV_GetColumns($aArray, $iRow)
	Local $sOut
	For $i = 0 To UBound($aArray, 2) - 1
		$sOut &= $aArray[$iRow][$i] & "   |"
	Next
	Return StringTrimRight($sOut, 1)
EndFunc   ;==>_CV_GetColumns


Func _CV_GetTcpTable($tSplash = 0)
	...
	For $i = 1 To $iTCPentries
		...
		Else
			...
			$aTCPTable[$i][1] = _CV_IpToName($iIP) & " (" & $aTCPTable[$i][1] & ")"
		EndIf
		...
		EndIf
		If $tSplash > 0 Then ControlSetText($tSplash, "", "Static1", $aStartText & "Found TCP process " & $i & @CRLF & $aTCPTable[$i][0] & " | " & $aTCPTable[$i][2])
...
EndFunc   ;==>_CV_GetTcpTable


Func _CV_GetExtendedTcpTable($tSplash = 0)
	...
	For $i = 1 To $iTCPentries
		...
		$aTCPTable[$i][2] &= _CV_GetPortHint($aTCPTable[$i][2])
		...
		Else
			...
			$aTCPTable[$i][4] &= _CV_GetPortHint($aTCPTable[$i][4])
		EndIf
...
Func _CV_GetUdpTable($tSplash)
	...
	For $i = 1 To $iUDPentries
		If $tSplash > 0 Then ControlSetText($tSplash, "", "Static1", $aStartText & "Scanning UDP port " & $i)
		...
		Else
			...
			$aUDPTable[$i][0] = _CV_IpToName($iIP) & " (" & $aUDPTable[$i][0] & ")"
		EndIf

		$aUDPTable[$i][1] = Dec(Hex(BinaryMid(DllStructGetData($tMIB_UDPTABLE_DWORDS, 1, $iOffset + 2), 1, 2)))

		If $tSplash > 0 Then ControlSetText($tSplash, "", "Static1", $aStartText & "Found UDP process " & $i & @CRLF & $aUDPTable[$i][0] & " | " & $aUDPTable[$i][2])
...
EndFunc   ;==>_CV_GetUdpTable


Func _CV_GetExtendedUdpTable($tSplash)
	If $tSplash > 0 Then ControlSetText($tSplash, "", "Static1", $aStartText & "Starting UDP port scan.")
	...
	For $i = 1 To $iUDPentries
		...
		If $iIP = 16777343 Then
			$aUDPTable[$i][1] = "localhost (127.0.0.1)"
		ElseIf $iIP = 0 Then
		...
		Else
			...
			$aUDPTable[$i][1] = _CV_IpToName($iIP) & " (" & $aUDPTable[$i][1] & ")"
		EndIf
...
Func _CV_DisableConnection($sConnectionInfoString)
	Local $aArrayOfData = StringSplit($sConnectionInfoString, "|", 3)
	If UBound($aArrayOfData) < 5 Then Return SetError(1, 0, 0)
...
Func _CV_Format($sInput)
	Local $aString = StringSplit($sInput, "|", 2)
	...
	Local $sOut = $sDay & ", " & @MDAY & " " & $sMon & " " & @YEAR & " " & @HOUR & ':' & @MIN & ':' & @SEC
	Switch UBound($aString)
		Case 3
			$sOut &= ", UDP Connection: " & @CRLF & _
					"    Local IP: " & $aString[0] & @CRLF & _
					"    Local Port: " & $aString[1] & @CRLF
		Case 6
			$sOut &= ", TCP Connection: " & @CRLF & _
					"    Connection State: " & $aString[0] & @CRLF & _
					"    Local IP: " & $aString[1] & @CRLF & _
					"    Local Port: " & $aString[2] & @CRLF & _
					"    Remote IP: " & $aString[3] & @CRLF & _
					"    Remote Port: " & $aString[4] & @CRLF
		Case 7
			$sOut &= ", UDP Connection: " & @CRLF & _
					"    Application Name: " & $aString[0] & @CRLF & _
					"    Local IP: " & $aString[1] & @CRLF & _
					"    Local Port: " & $aString[2] & @CRLF & _
					"    PID: " & $aString[3] & @CRLF & _
					"    Full Path of the application: " & $aString[4] & @CRLF & _
					"    User Name: " & $aString[5]
		Case 10
			$sOut &= ", TCP Connection: " & @CRLF & _
					"    Application Name: " & $aString[0] & @CRLF & _
					...
					"    Local Port: " & $aString[2] & @CRLF & _
					"    Remote IP: " & $aString[3] & @CRLF & _
					"    Remote Port: " & $aString[4] & @CRLF & _
					"    Connection State: " & $aString[5] & @CRLF & _
					"    PID: " & $aString[6] & @CRLF & _
					"    Full Path of the application: " & $aString[7] & @CRLF & _
					"    User Name: " & $aString[8]
	EndSwitch
	Return $sOut
EndFunc   ;==>_CV_Format
...
Func _CV_CreateDragDots($hGUI)
	...
	Return $aCall[0]
EndFunc   ;==>_CV_CreateDragDots


Func _CV_AdjustPos($hWnd, $iMsg, $wParam, $lParam)
	#forceref $iMsg
	...
	If $hWnd = $hGUI Then
		...
		EndSwitch
	EndIf
EndFunc   ;==>_CV_AdjustPos


Func _CV_SetMinMax($hWnd, $iMsg, $wParam, $lParam)
